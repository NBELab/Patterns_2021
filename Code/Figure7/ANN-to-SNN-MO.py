...
##################################################################################
MODEL_NAME = '../../Data/fc200kenergy2.0cm_obstcl_20210515-233649_5_128_relu_0_10_0.3_0.1_2.5_lrsched2.h5'
##################################################################################
ENABLE_LOG = 1
VERBOSE = 1
...
#
#############################################################################
def CheckCollisions(angles , truthEePos):
    ...
    for_counter = 0
    for sample in angles: #angles
        sampleq = np.reshape(sample, (5,1))
        Js = arm.calculate_Js(sampleq) #all joints angles
        result = IsArmIntersect(Js, OBSTACLE_CENTER_POS_IN_M, OBSTACLE_RADIUS_IN_M)
        isbeyond = IsPosBeyondReach(truthEePos[for_counter], OBSTACLE_CENTER_POS_IN_M, OBSTACLE_RADIUS_IN_M)
        if isbeyond:
            points_beyond_reach = points_beyond_reach + 1        

        if result == True:
            count_intersections = count_intersections + 1
            colided_ee_points = np.append(colided_ee_points, truthEePos[for_counter]) #original xyz
    ...
    return count_intersections, points_beyond_reach_intersect, points_beyond_reach
#end 
...
def PredictAndCalcFkMse(arm, allDataNonNorm, truthEePos, predAngles, normalize=True, plotFile="plot.png", showPlot=False):
    ...
    distance_func = lambda t: (np.square(t[0]-OBSTACLE_CENTER_POS_IN_M[0])+np.square(t[1]-OBSTACLE_CENTER_POS_IN_M[1])+np.square(t[2]-OBSTACLE_CENTER_POS_IN_M[2])) <= OBSTACLE_RADIUS_IN_M**2
    squares = np.array([distance_func(xi) for xi in truthEePos])
    insideObstacle = truthEePos[squares] 
    ...
    count_intersections, points_beyond_reach_intersect, points_beyond_reach = CheckCollisions(DeNormalizedAngles,truthEePos)
    ...
    print('Average Euclidean Distance to target in Meters: {}, min distance: {}, \
        max distance: {} Points above 1 cm distance: {} ; Points above 1 mm distance: {} ; Points inside obstacle: {} ; Points Intersect: {} ; Points Beyond Reach Intersect: {} ; Points Beyond Reach: {}\n'.format(BK.mean(eloss, axis=0), 
        BK.min(eloss, axis=0), BK.max(eloss, axis=0), np.size(largerThan1CmArrayPred,0),np.size(largerThan1MmArrayPred,0),
        len(insideObstacle), count_intersections, points_beyond_reach_intersect, points_beyond_reach ))
...
if LOAD_MODEL:
    ...
    for l in range(START_LAYER, END_LAYER):
        ...
        if DO_NENGO:
            syn = [0.02]#[0.011, 0.018, 0.02, 0.022, 0.025, 0.03]
            fr =  [5000]#[5000, 1500, 1000, 750, 500, 250]
            for f in (fr):
                for i in syn:
                    print("\n####################### Spiking Net #######################\n", i, f)
                    netPrediction = RunNetwork(
