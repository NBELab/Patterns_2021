...
def test_NN_IK_configuration(num_layers, num_ftrs, activation, use_angle_norm, batch_size):
    ...
    name = 'resnet200k_mae_{num_layers}_{num_ftrs}_{activ}_{theta_norm}_{bs}_lrsched2'.\
...
###############################################################################################################
    ...
    model = ResnetFC(num_layers, num_ftrs)
...
#        orig_angles = orig_angles.reshape((-1,5))
        recovered_angles = recovered_angles.reshape((-1,5)) #None #
